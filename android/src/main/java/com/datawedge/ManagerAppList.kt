...
class ManagerAppList {
  ...
  fun addProfile(profile: ZebraProfile) {
    if (profile.profileName == this.profileName) this.rightProfile = profile

    if (profile.profileName != this.profileName) this.wrongProfile = profile
  ...
  private fun addAppOnRightProfile() {
    ...
    val newAppList = appList.map { item -> createAppListBundle(item) }.toMutableList()
  ...
  private fun createAppListBundle(bundle: Bundle): Bundle {
    ...
    val activities = bundle.getStringArrayList("ACTIVITY_LIST")?.toTypedArray()
